...
float l = 0.2;		//Rest length of spring
...
SceneShader::SceneShader(): Shader()
{
	...
	for (int i = 0; i < 5; i++)
		for (int j = 0; j < 5; j++)
			for (int k = 0; k < 5; k++){
				int c = (i + 6 * (j + 6 * k));
				P[c].x = glm::vec3((i*0.2), (2+j*0.2), (k*0.2));
				P[c].v = glm::vec3(0.0, 0.0, 0.0);
				P[c].F = glm::vec3(0.0, 0.0, 0.0);
				P[c].Fs = glm::vec3(0.0, 0.0, 0.0);
				P[c].Fd = glm::vec3(0.0, 0.0, 0.0);
				P[c].m = 1.0;
		}
...
void SceneShader::simulation(float t) {
	...
	for (int i = 0; i < nMass; i++){
		...
		float tmp = dif / dis;
		P[i].Fs = glm::vec3(0.0, (dis - l), 0.0) * (dif / dis)*ks;
		P[i].Fd = -kd*P[i].v;
		float tmp1 = glm::dot((P[i].v), (P[i].Fs / (glm::length(P[i].Fs))));
		float tmp2 = glm::dot((P[i].Fs / (glm::length(P[i].Fs))), (P[i].Fs / (glm::length(P[i].Fs))));
		//P[i].Fd = -kd*(tmp1/tmp2)*(P[i].Fs / (glm::length(P[i].Fs)));
	...
	for (int i = 0; i < nMass; i++){
		if (((P[i].m) > 0) && (CF > 0)){							//If not fixed
			...
			P[i].v = P[i].v + ((G*P[i].m) / (P[i].m))*dt;
			P[i].x = P[i].x + CF*dt*P[i].v;
			//P[i].x = P[i].x + dt*G;
		}
